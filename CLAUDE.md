...
## 컨텍스트 핸드오프 (IMPORTANT)
매 프롬프트 처리가 끝날 때마다 `memory-bank/context.md` 파일을 항상 최신 상태로 갱신한다
(파일이 없으면 새로 생성). 새 세션/에이전트가 이 파일만 읽고도 프로젝트 현재 상태를 이어받을
수 있도록 아래 내용을 담는다.
- 현재 진행 중인 작업과 그 배경/확정된 결정 사항
- 구현 상태(무엇이 끝났고 무엇이 안 끝났는지, 코드 기준으로 검증된 사실)
- 재조사 없이 바로 쓸 수 있는 핵심 조사 결과(파일 경로/라인 번호 등 구체적으로)
- 다음에 할 일
`memory-bank/planAndTask.md`(작업계획)와 역할이 다르다: `planAndTask.md`는 작업 항목의
목록/완료 여부를 추적하고, `context.md`는 "새 세션이 지금 상황을 파악하기 위한 요약"에
집중한다. 단순 질의응답처럼 프로젝트 상태에 변화가 없는 요청이면 갱신을 생략해도 된다.
