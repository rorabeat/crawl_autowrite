...
## 다음 할 일
- 사용자가 실제로 다시 발행을 돌려서 좀비 타깃 복구 로직(`_connect_over_cdp_with_retry`)이
  실제 환경에서 정상 동작하는지 확인 필요(라이브 검증 대기).
- `memory-bank/planAndTask.md`에 위 두 수정(계정 전환 경쟁 조건, CDP 좀비 타깃)을 별도
  절/번호로 정리해 넣을 것 — 아직 반영 안 됨.
...
## 커밋 상태
위 "크롬 CDP 좀비 타깃으로 인한 발행 실패 수정"(`NaverAutoWrite/naver_login.py`)은
