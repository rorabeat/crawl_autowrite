...
### 15. 다중 네이버 계정 발행 (Task 019)

- [ ] "계정 관리" 다이얼로그에서 계정을 추가하면 목록에 즉시 반영되고, 앱을 재시작해도
      유지된다(`accounts.json`)
- [ ] 계정을 수정/삭제하면 즉시 반영되고, 삭제된 계정을 참조하던 태스크는 발행 시 경고
      로그만 남기고 기본 계정(.env)으로 폴백한다(앱이 죽지 않음)
- [ ] 입력 탭/태스크 편집/대기열 일괄 설정에서 고른 "발행 계정"이 각각 독립적으로
      저장되고, 멀티 작업 대기열 목록에 계정 라벨이 표시된다
- [ ] (실 계정 2개 필요) 서로 다른 계정으로 지정한 태스크 2개를 순서대로 대기열에 넣고
      실행하면, 각 발행이 실제로 지정한 계정의 블로그에 등록된다(자격증명이 섞이지 않음)
- [ ] (실 계정 2개 필요) 위 상황에서 두 번째 발행 직전에 크롬이 재시작되는지 확인한다
      (작업 관리자에서 첫 번째 발행이 띄운 chrome.exe 프로세스가 종료되고 새로 뜸).
      같은 계정으로 연속 발행할 때는 크롬이 재시작되지 않는지도 확인한다

### 16. 진행 중 작업 취소 및 대기 작업 우선순위 변경 (Task 020)

- [ ] "멀티 작업" 탭에서 작업이 진행 중일 때 "진행 중 작업 취소" 버튼을 누르면 확인
      다이얼로그가 뜨고, 확인하면 목록 상태가 즉시 "취소 중…"으로 바뀐다
- [ ] 잠시 후(늦어도 몇 초 이내) 진행 중이던 서브프로세스(크롤링/`codex exec`/발행)가
      실제로 종료되고(작업 관리자에서 확인), "진행 중 작업" 목록에서 항목이 사라지며
      진행 로그에 "취소됨"이 표시된다
- [ ] 취소 요청 후 "진행 중 작업 취소" 버튼이 비활성화되어 중복 클릭이 안 된다
- [ ] 진행 중인 작업이 없을 때 취소를 시도하면 안내 로그만 뜨고 오류 없이 무시된다
- [ ] "대기 중 작업" 목록에서 작업을 선택해 "위로"/"아래로"를 누르면 목록 순서가
      바뀌고, 대기 중이던 작업이 실제로 바뀐 순서대로 실행된다
- [ ] 맨 위/맨 아래 항목에서 각각 "위로"/"아래로"를 눌러도 순서가 바뀌지 않고 오류가
      발생하지 않는다
- [ ] 앱을 재시작해도 취소된 작업은 대기열에 남아있지 않다(`queue_state.json` 확인)
